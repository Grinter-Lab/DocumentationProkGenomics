...
If you have [conda](https://conda.io/projects/conda/en/latest/user-guide/getting-started.html) installed in your system (either in your **computer** or your **server**), you could create a conda environment and install nextflow following the following commands
...
This pipeline allows you to run several tools without the need of installing each tool by yourself. It uses **conda, docker or singularity** to create environments or containers with the necessary dependencies and tools. At the moment, the option of running this pipeline with conda is susceptible to break in some conda versions or conda with customised channels in its configuration. Therefore the best option for now is to run it with singularity.

In your **cluster** you need to load **singularity** (successfully tested on singularity-ce version 3.9.9-focal). Please call the module. Type the following command and use `tab` after the word `singularity` to see if you have different versions available.  
